        ...
        if request.method == 'POST':
            if 'passwordUpdate' in request.POST:
                ...
                if form.is_valid():
                    ...
                    messages.error(request,'Password Update failed!', extra_tags='password' )
            
            elif 'userUpdate' in request.POST:
                form_user = updateUserForm(request.POST, instance=user)
                if form_user.is_valid():
                    form_user.save()
                    messages.success(request, 'User updated!', extra_tags='user')
                else:
                    messages.error(request, 'User update failed!', extra='user')
                    form_user = updateUserForm(instance=user)
    
        context = {'user': user, 'form':form, 'form_user':form_user }
